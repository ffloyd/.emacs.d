...
Default /modeline/ looks too simple and distracting at the same
time. I want something lightweight, laconic and fancy. [[https://seagle0128.github.io/doom-modeline/][doom-modeline]]
...
I want to see position like ~123:13~ instead of ~L123~ in the
modeline.
...
** Saving and loading /bookmark/ lists
I usually use bookmarks to remember places in the code that related to
my current task. When I work on multiple things during the day I do
not want to mix bookmarks for different tasks. So I need some bookmark
lists management solution.
...
By default ~xref~ reuses the same buffer for results. But I want to
create a new xref buffer for each "find references" command.
